...
public class Lesson07Ex02RestController {
	...
	public List<StudentEntity> select1() {
		...
		// 5) in문으로 일치하는 값 모두 조회 (찾는 이름이 여러개인 경우)
		// list.of => list add를 한거랑 동일
		// return studentRepository.findByNameIn(List.of("조세호", "유재석"));

		// 6) 여러 컬럼값과 일치하는 데이터 조회 (AND)
		// return studentRepository.findByNameAndDreamJob("홍길동", "의사");
